...
async function run() {
  ...
  const cancelA = await request("/api/admin/orders/" + encodeURIComponent(f.orderAId) + "/status", json("PATCH", { status: "cancelled", note: "CI integrity cancellation A" }, mainCookie)); assert.equal(cancelA.response.status, 200); const cancelledAIntegrity = await inspect(); const stockCancelled = stockOf(cancelledAIntegrity, f.variantAId); assert.equal(stockCancelled, stockBefore); assert.equal(cancelledAIntegrity.movements.filter((x) => x.variantId === f.variantAId && x.reason === "order_cancel_restore" && x.referenceId === f.orderAId).length, 1); console.log("STOCK AFTER CANCELLATION: " + stockCancelled); console.log("STOCK RESTORED ONCE: PASS");
  ...
  const repeatB = await request("/api/admin/orders/" + encodeURIComponent(f.orderBId) + "/status", json("PATCH", { status: "payment_confirmed" }, mainCookie)); assert.ok(repeatB.response.status >= 400); assert.equal((await inspect()).jobs.filter((x) => x.orderId === f.orderBId).length, 1);
  const finalA = await request("/api/admin/orders/" + encodeURIComponent(f.orderAId), { headers: { cookie: mainCookie } }); const finalB = await request("/api/admin/orders/" + encodeURIComponent(f.orderBId), { headers: { cookie: mainCookie } }); assert.equal(finalA.response.status, 200); assert.equal(finalB.response.status, 200); assert.ok(finalA.data.order.history.some((x) => x.toStatus === "payment_confirmed")); assert.ok(finalA.data.order.history.some((x) => x.toStatus === "cancelled")); assert.ok(finalB.data.order.history.some((x) => x.toStatus === "payment_confirmed")); console.log("DUPLICATE PRINTING JOB: NO"); console.log("ORDER HISTORY: PASS");
  const operatorOrderB = await request("/api/admin/orders/" + encodeURIComponent(f.orderBId), { headers: { cookie: operatorCookie } }); assert.equal(operatorOrderB.response.status, 200); assert.equal(operatorOrderB.data.order.id, f.orderBId); const operatorQueueB = await request("/api/admin/production?search=" + encodeURIComponent(f.orderBId), { headers: { cookie: operatorCookie } }); assert.equal(operatorQueueB.response.status, 200); assert.ok(operatorQueueB.data.items.some((x) => x.orderId === f.orderBId && x.jobId === job.jobId)); console.log("PRINTING OPERATOR REAL ORDER ACCESS: PASS");
