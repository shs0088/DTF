...
export class OrdersFixtureStore extends ItemStore {
  async ciCreateFixtures(input: { actorId: string; runId: string }): Promise<any> {
    ...
    return { runId, orderAId: orders.a.id, orderRId: orders.r.id, orderBId: orders.b.id, variantAId: variants.a, variantRId: variants.r, variantBId: variants.b, orderedQuantityA: 1, orderedQuantityR: 1, orderedQuantityB: 1, designId, approvedMasterId: assetId, orderItemBId: orders.b.items?.[0]?.id };
  }
