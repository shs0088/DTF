...
export class ItemStore extends DurableObject<ItemStoreEnv> {
  ...
  private bootstrapCatalog(): void {
    this.ctx.storage.sql.exec(`
      ...
      CREATE TABLE IF NOT EXISTS admin_user_group_memberships (
        user_id TEXT PRIMARY KEY REFERENCES admin_users(id) ON DELETE CASCADE,
        group_id TEXT NOT NULL REFERENCES admin_user_groups(id)
      );
      ...
      INSERT OR IGNORE INTO admin_user_groups (id,name,is_system,full_access,enabled) VALUES
        ('main_admin','Main Administrator',1,1,1),
        ('printing_technician','Printing Operator',1,0,1);
      INSERT OR IGNORE INTO admin_group_permissions (group_id,resource,can_access,can_modify) VALUES
        ('main_admin','admin.dashboard',1,1),
        ('main_admin','admin.orders',1,1),
        ('main_admin','admin.production',1,1),
        ('main_admin','admin.products.printify',1,1),
        ('main_admin','admin.users',1,1),
        ('main_admin','admin.user_groups',1,1),
        ('main_admin','admin.settings',1,1),
        ('printing_technician','admin.dashboard',1,0),
        ('printing_technician','admin.orders',1,1),
        ('printing_technician','admin.production',1,1);
      INSERT OR IGNORE INTO admin_user_group_memberships (user_id,group_id)
        SELECT id, CASE WHEN role='main_admin' THEN 'main_admin' ELSE 'printing_technician' END FROM admin_users;
  ...
  async adminIdentity(id: string): Promise<{id:string;username:string;role:"main_admin"|"printing_technician";groupId:string}|null> { this.bootstrapCatalog(); const row=this.ctx.storage.sql.exec<any>("SELECT u.id,u.username,u.role,COALESCE(m.group_id,CASE WHEN u.role='main_admin' THEN 'main_admin' ELSE 'printing_technician' END) AS groupId FROM admin_users u LEFT JOIN admin_user_group_memberships m ON m.user_id=u.id WHERE u.id=? AND u.enabled=1",id).toArray()[0]; return row ? {id:String(row.id),username:String(row.username),role:row.role,groupId:String(row.groupId)} : null; }
  ...
  private assertMainAdmin(actorId: string): void { const actor=this.ctx.storage.sql.exec<any>("SELECT u.id,u.enabled,COALESCE(m.group_id,CASE WHEN u.role='main_admin' THEN 'main_admin' ELSE 'printing_technician' END) AS groupId FROM admin_users u LEFT JOIN admin_user_group_memberships m ON m.user_id=u.id WHERE u.id=?",actorId).toArray()[0]; if(!actor || Number(actor.enabled)!==1 || actor.groupId!=="main_admin") throw new Error("Main Administrator permission required."); }
  private assertAdminPermission(actorId: string, resource: string, mode: "access"|"modify"): void { const actor=this.ctx.storage.sql.exec<any>("SELECT id,enabled FROM admin_users WHERE id=?",actorId).toArray()[0]; if(!actor || Number(actor.enabled)!==1 || !this.adminCanUser(actorId,resource,mode)) throw new Error("Admin permission required."); }
  private adminGroupRow(groupId: string): any { return this.ctx.storage.sql.exec<any>("SELECT id,name,is_system AS isSystem,full_access AS fullAccess,enabled FROM admin_user_groups WHERE id=?",groupId).toArray()[0]; }
  ...
  async createAdminUserAsync(actorId: string, username: string, password: string, groupId: string): Promise<unknown> { this.bootstrapCatalog(); this.assertAdminPermission(actorId,"admin.users","modify"); const clean=String(username||"").trim().toLowerCase(); if(!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(clean)) throw new Error("Username must be 3-64 characters and use letters, numbers, dot, underscore, or hyphen."); if(String(password||"").length<12) throw new Error("Password must be at least 12 characters."); const group=this.adminGroupRow(String(groupId||"")); if(!group || Number(group.enabled)!==1) throw new Error("Invalid or disabled Admin User Group."); if(this.ctx.storage.sql.exec<any>("SELECT id FROM admin_users WHERE username=?",clean).toArray()[0]) throw new Error("Username is already in use."); const salt=crypto.getRandomValues(new Uint8Array(16)); const hash=await this.adminPasswordHash(password,salt); const id=crypto.randomUUID(); const role=String(group.id)==="main_admin"?"main_admin":"printing_technician"; this.ctx.storage.sql.exec("INSERT INTO admin_users (id,username,password_salt,password_hash,role) VALUES (?,?,?,?,?)",id,clean,this.adminB64(salt),hash,role); this.ctx.storage.sql.exec("INSERT INTO admin_user_group_memberships (user_id,group_id) VALUES (?,?)",id,String(group.id)); this.adminAudit(actorId,"admin.user.create","admin_user",id,"success",{username:clean,groupId:String(group.id)}); return {id,username:clean,role,groupId:String(group.id),enabled:1}; }
  setAdminUserEnabled(actorId: string, targetId: string, enabled: boolean): unknown { this.bootstrapCatalog(); this.assertAdminPermission(actorId,"admin.users","modify"); const row=this.ctx.storage.sql.exec<any>("SELECT u.id,u.enabled,COALESCE(m.group_id,CASE WHEN u.role='main_admin' THEN 'main_admin' ELSE 'printing_technician' END) AS groupId FROM admin_users u LEFT JOIN admin_user_group_memberships m ON m.user_id=u.id WHERE u.id=?",targetId).toArray()[0]; if(!row) throw new Error("Admin user not found."); if(!enabled && row.groupId==="main_admin" && Number(this.ctx.storage.sql.exec<any>("SELECT COUNT(*) AS count FROM admin_users u JOIN admin_user_group_memberships m ON m.user_id=u.id WHERE m.group_id='main_admin' AND u.enabled=1").one().count)<=1) throw new Error("The last Main Administrator cannot be disabled."); this.ctx.storage.sql.exec("UPDATE admin_users SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",enabled?1:0,targetId); this.adminAudit(actorId,"admin.user.set_enabled","admin_user",targetId,"success",{enabled}); return {id:targetId,enabled:enabled?1:0}; }
  setAdminUserGroup(actorId: string, targetId: string, groupId: string): unknown { this.bootstrapCatalog(); this.assertAdminPermission(actorId,"admin.users","modify"); const row=this.ctx.storage.sql.exec<any>("SELECT u.id,u.enabled,COALESCE(m.group_id,CASE WHEN u.role='main_admin' THEN 'main_admin' ELSE 'printing_technician' END) AS groupId FROM admin_users u LEFT JOIN admin_user_group_memberships m ON m.user_id=u.id WHERE u.id=?",targetId).toArray()[0]; if(!row) throw new Error("Admin user not found."); const group=this.adminGroupRow(groupId); if(!group || Number(group.enabled)!==1) throw new Error("Invalid or disabled Admin User Group."); if(row.groupId==="main_admin"&&groupId!=="main_admin"&&Number(row.enabled)===1&&Number(this.ctx.storage.sql.exec<any>("SELECT COUNT(*) AS count FROM admin_users u JOIN admin_user_group_memberships m ON m.user_id=u.id WHERE m.group_id='main_admin' AND u.enabled=1").one().count)<=1) throw new Error("The last Main Administrator cannot be moved from the Main Administrator group."); const role=groupId==="main_admin"?"main_admin":"printing_technician"; this.ctx.storage.sql.exec("INSERT OR REPLACE INTO admin_user_group_memberships (user_id,group_id) VALUES (?,?)",targetId,groupId); this.ctx.storage.sql.exec("UPDATE admin_users SET role=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",role,targetId); this.adminAudit(actorId,"admin.user.set_group","admin_user",targetId,"success",{groupId}); return {id:targetId,groupId,role}; }
  setAdminUserRole(actorId: string, targetId: string, role: "main_admin"|"printing_technician"): unknown { return this.setAdminUserGroup(actorId,targetId,role); }
  ...
  adminCan(role: "main_admin"|"printing_technician", resource: string, action: string): boolean { this.bootstrapCatalog(); if(role==="main_admin") return true; const mode=action==="access"?"access":"modify"; const group=this.adminGroupRow("printing_technician"); if(!group || Number(group.enabled)!==1) return false; const row=this.ctx.storage.sql.exec<any>("SELECT can_access AS canAccess,can_modify AS canModify FROM admin_group_permissions WHERE group_id='printing_technician' AND resource=?",resource).toArray()[0]; return mode==="access"?Number(row?.canAccess)===1:Number(row?.canModify)===1; }
  adminUsers() { this.bootstrapCatalog(); return this.ctx.storage.sql.exec("SELECT u.id,u.username,u.role,u.enabled,COALESCE(m.group_id,CASE WHEN u.role='main_admin' THEN 'main_admin' ELSE 'printing_technician' END) AS groupId,COALESCE(g.name,CASE WHEN u.role='main_admin' THEN 'Main Administrator' ELSE 'Printing Operator' END) AS groupName,u.created_at AS createdAt,u.updated_at AS updatedAt,u.last_login_at AS lastLoginAt FROM admin_users u LEFT JOIN admin_user_group_memberships m ON m.user_id=u.id LEFT JOIN admin_user_groups g ON g.id=m.group_id ORDER BY u.username").toArray(); }
  adminGroups() { this.bootstrapCatalog(); return this.ctx.storage.sql.exec("SELECT g.id,g.name,g.is_system AS isSystem,g.full_access AS fullAccess,g.enabled,COUNT(m.user_id) AS userCount FROM admin_user_groups g LEFT JOIN admin_user_group_memberships m ON m.group_id=g.id GROUP BY g.id,g.name,g.is_system,g.full_access,g.enabled ORDER BY g.is_system DESC,g.name").toArray(); }
  adminPermissionMatrix(groupId: string) { this.bootstrapCatalog(); return this.ctx.storage.sql.exec("SELECT p.resource,p.can_access AS canAccess,p.can_modify AS canModify FROM admin_group_permissions p WHERE p.group_id=? ORDER BY p.resource",groupId).toArray(); }
  createAdminGroup(actorId: string, name: string): unknown { this.bootstrapCatalog(); this.assertAdminPermission(actorId,"admin.user_groups","modify"); const clean=String(name||"").trim(); if(clean.length<3||clean.length>80) throw new Error("Group name must be 3-80 characters."); if(this.ctx.storage.sql.exec<any>("SELECT id FROM admin_user_groups WHERE lower(name)=lower(?)",clean).toArray()[0]) throw new Error("User Group name already exists."); const id="grp-"+crypto.randomUUID(); this.ctx.storage.sql.exec("INSERT INTO admin_user_groups (id,name,is_system,full_access,enabled) VALUES (?,?,0,0,1)",id,clean); for(const item of this.adminResources()) this.ctx.storage.sql.exec("INSERT OR IGNORE INTO admin_group_permissions (group_id,resource,can_access,can_modify) VALUES (?,?,0,0)",id,item.resource); this.adminAudit(actorId,"admin.group.create","admin_user_group",id,"success",{name:clean}); return {id,name:clean,isSystem:0,fullAccess:0,enabled:1,userCount:0}; }
  updateAdminGroup(actorId: string, groupId: string, input: {name?:string;enabled?:boolean;access?:string[];modify?:string[]}): unknown { this.bootstrapCatalog(); this.assertAdminPermission(actorId,"admin.user_groups","modify"); const group=this.adminGroupRow(groupId); if(!group) throw new Error("Admin User Group not found."); if(Number(group.isSystem)===1 && (input.name!==undefined || input.enabled===false)) throw new Error("Protected system groups cannot be renamed or disabled."); if(input.name!==undefined){const clean=String(input.name).trim(); if(clean.length<3||clean.length>80) throw new Error("Group name must be 3-80 characters."); if(this.ctx.storage.sql.exec<any>("SELECT id FROM admin_user_groups WHERE lower(name)=lower(?) AND id<>?",clean,groupId).toArray()[0]) throw new Error("User Group name already exists."); this.ctx.storage.sql.exec("UPDATE admin_user_groups SET name=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",clean,groupId);} if(input.enabled!==undefined) this.ctx.storage.sql.exec("UPDATE admin_user_groups SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",input.enabled?1:0,groupId); if(Number(group.fullAccess)!==1 && (input.access||input.modify)){const access=new Set((input.access||[]).map(String)); const modify=new Set((input.modify||[]).map(String)); for(const item of this.adminResources()){const canAccess=access.has(item.resource)?1:0; const canModify=modify.has(item.resource)?1:0; this.ctx.storage.sql.exec("INSERT OR REPLACE INTO admin_group_permissions (group_id,resource,can_access,can_modify) VALUES (?,?,?,?)",groupId,item.resource,canAccess,canModify);} } this.adminAudit(actorId,"admin.group.update","admin_user_group",groupId,"success",input); return {id:groupId}; }
  deleteAdminGroup(actorId: string, groupId: string): unknown { this.bootstrapCatalog(); this.assertAdminPermission(actorId,"admin.user_groups","modify"); const group=this.adminGroupRow(groupId); if(!group) throw new Error("Admin User Group not found."); if(Number(group.isSystem)===1) throw new Error("Protected system groups cannot be deleted."); const count=Number(this.ctx.storage.sql.exec<any>("SELECT COUNT(*) AS count FROM admin_user_group_memberships WHERE group_id=?",groupId).one().count); if(count>0) throw new Error("Cannot delete a User Group while Admin users are assigned to it."); this.ctx.storage.sql.exec("DELETE FROM admin_group_permissions WHERE group_id=?",groupId); this.ctx.storage.sql.exec("DELETE FROM admin_user_groups WHERE id=?",groupId); this.adminAudit(actorId,"admin.group.delete","admin_user_group",groupId,"success",{}); return {id:groupId,deleted:true}; }
