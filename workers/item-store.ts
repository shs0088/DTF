...
export class ItemStore extends DurableObject<ItemStoreEnv> {
  ...
  private assertMainAdmin(actorId: string): void { const actor=this.ctx.storage.sql.exec<any>("SELECT id,role,enabled FROM admin_users WHERE id=?",actorId).toArray()[0]; if(!actor || Number(actor.enabled)!==1 || actor.role!=="main_admin") throw new Error("Main Administrator permission required."); }
  async createAdminUserAsync(actorId: string, username: string, password: string, role: "main_admin"|"printing_technician"): Promise<unknown> { this.bootstrapCatalog(); this.assertMainAdmin(actorId); const clean=String(username||"").trim().toLowerCase(); if(!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(clean)) throw new Error("Username must be 3-64 characters and use letters, numbers, dot, underscore, or hyphen."); if(String(password||"").length<12) throw new Error("Password must be at least 12 characters."); if(role!=="main_admin"&&role!=="printing_technician") throw new Error("Invalid admin group."); if(this.ctx.storage.sql.exec<any>("SELECT id FROM admin_users WHERE username=?",clean).toArray()[0]) throw new Error("Username is already in use."); const salt=crypto.getRandomValues(new Uint8Array(16)); const hash=await this.adminPasswordHash(password,salt); const id=crypto.randomUUID(); this.ctx.storage.sql.exec("INSERT INTO admin_users (id,username,password_salt,password_hash,role) VALUES (?,?,?,?,?)",id,clean,this.adminB64(salt),hash,role); this.adminAudit(actorId,"admin.user.create","admin_user",id,"success",{username:clean,role}); return {id,username:clean,role,enabled:1}; }
  setAdminUserEnabled(actorId: string, targetId: string, enabled: boolean): unknown { this.bootstrapCatalog(); this.assertMainAdmin(actorId); const row=this.ctx.storage.sql.exec<any>("SELECT id,role FROM admin_users WHERE id=?",targetId).toArray()[0]; if(!row) throw new Error("Admin user not found."); if(!enabled && row.role==="main_admin" && Number(this.ctx.storage.sql.exec<any>("SELECT COUNT(*) AS count FROM admin_users WHERE role='main_admin' AND enabled=1").one().count)<=1) throw new Error("The last Main Administrator cannot be disabled."); this.ctx.storage.sql.exec("UPDATE admin_users SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",enabled?1:0,targetId); this.adminAudit(actorId,"admin.user.set_enabled","admin_user",targetId,"success",{enabled}); return {id:targetId,enabled:enabled?1:0}; }
  setAdminUserRole(actorId: string, targetId: string, role: "main_admin"|"printing_technician"): unknown { this.bootstrapCatalog(); this.assertMainAdmin(actorId); const row=this.ctx.storage.sql.exec<any>("SELECT id,role FROM admin_users WHERE id=?",targetId).toArray()[0]; if(!row) throw new Error("Admin user not found."); if(row.role==="main_admin"&&role!=="main_admin"&&Number(this.ctx.storage.sql.exec<any>("SELECT COUNT(*) AS count FROM admin_users WHERE role='main_admin' AND enabled=1").one().count)<=1) throw new Error("The last Main Administrator cannot be demoted."); this.ctx.storage.sql.exec("UPDATE admin_users SET role=?,group_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",role,role==="main_admin"?"group-main-admin":"group-printing-operator",targetId); this.adminAudit(actorId,"admin.user.set_role","admin_user",targetId,"success",{role}); return {id:targetId,role}; }
  ...
  adminGroupPermissions(groupId: string): unknown[] {
 ...
 this.bootstrapCatalog(); return this.ctx.storage.sql.exec<any>("SELECT resource,permission FROM admin_group_permissions WHERE group_id=? ORDER BY resource,permission",groupId).toArray(); }
  ...
  setAdminUserGroup(actorId: string,targetId: string,groupId: string): unknown { this.assertMainAdmin(actorId); const target=this.ctx.storage.sql.exec<any>("SELECT u.id,u.role,u.group_id,g.protected FROM admin_users u LEFT JOIN admin_user_groups g ON g.id=u.group_id WHERE u.id=?",targetId).toArray()[0]; const group=this.ctx.storage.sql.exec<any>("SELECT id FROM admin_user_groups WHERE id=? AND enabled=1",groupId).toArray()[0]; if(!target||!group) throw new Error("Admin or group not found."); if(target.role==='main_admin'&&target.group_id==='group-main-admin'&&groupId!=='group-main-admin'&&Number(this.ctx.storage.sql.exec<any>("SELECT COUNT(*) AS count FROM admin_users WHERE role='main_admin' AND enabled=1 AND group_id='group-main-admin'").one().count)<=1) throw new Error("The last Main Administrator cannot leave the protected group."); this.ctx.storage.sql.exec("UPDATE admin_users SET group_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",groupId,targetId); this.adminAudit(actorId,"admin.user.assign_group","admin_user",targetId,"success",{groupId}); return this.adminGroupForUser(targetId); }
