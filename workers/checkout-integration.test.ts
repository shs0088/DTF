...
describe("Database-authoritative checkout integration contracts", () => {
  test("auth and cart identities use separate cookies in active routes", async () => {
    ...
    expect(cart).toContain("dtf_cart_session");
    expect(customize).toContain("dtf_cart_session");
  ...
  test("cart and checkout consume authoritative business settings rather than hard-coded delivery values", async () => {
    const cart=await Bun.file(new URL("../app/routes/cart.tsx",import.meta.url)).text();
    const checkout=await Bun.file(new URL("./item-store.ts",import.meta.url)).text();
    expect(cart).toContain("businessSettingsSnapshot()");
    expect(cart).toContain("standardDeliveryFee");
    expect(cart).toContain("freeDeliveryThreshold");
    expect(checkout).toContain("(this.businessSettingsSnapshot() as any).settings");
  });
});
