...
export async function loader({request,context}:Route.LoaderArgs){
  const url=new URL(request.url),store=itemStore(context),sessionId=cookieValue(request,"dtf_session"),locale=localeFromRequest(request);
  ...
  try{preview=await store.checkoutPreview(cartKey,coupon,fulfillment);}
    ...
    if(coupon){couponError=pick(locale,"Coupon validation failed.","تعذر التحقق من القسيمة.");preview=await store.checkoutPreview(cartKey,"",fulfillment);}
...
export async function action({request,context}:Route.ActionArgs){
  const form=await request.formData(),store=itemStore(context),locale=localeFromRequest(request);
  ...
  try{
    ...
    if(!order?.id) return {ok:false,error:pick(locale,"Order could not be created.","تعذر إنشاء الطلب.")};
    throw redirect("/order/"+encodeURIComponent(String(order.id)));
  }catch(error){
    if(error instanceof Response) throw error;
    return {ok:false,error:pick(locale,"Checkout failed.","فشل إتمام الطلب.")};
  }
}
