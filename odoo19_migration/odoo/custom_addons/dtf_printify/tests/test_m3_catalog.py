...
class TestDTFM3Catalog(TransactionCase):
    def test_native_public_category_and_product_extensions(self):
        ...
        product = self.env["product.template"].create({"name": "DTF Shirt", "public_categ_ids": [(6, 0, [category.id])], "dtf_product_type": "tshirt", "dtf_catalog_type": "customizable", "dtf_print_your_dream_eligible": True, "is_published": False})
    ...
    def test_native_variants_and_stock_are_used(self):
        ...
        product = self.env["product.template"].create({"name": "Variant Shirt", "attribute_line_ids": [(0, 0, {"attribute_id": attribute.id, "value_ids": [(6, 0, [value.id])]})]})
    ...
    def test_printify_mapping_and_native_publication(self):
        ...
        self.assertFalse(product.is_published)
        self.assertNotIn("bp-1", str(product.dtf_public_payload(product)))
    ...
    def test_printify_access_is_admin_only(self):
        ...
        designer = self.env["res.users"].with_context(no_reset_password=True).create({"name": "Designer", "login": "m3-designer@example.test", "group_ids": [(6, 0, [designer_group.id])]})
