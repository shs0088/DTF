...
class TestDTFM3Catalog(TransactionCase):
    def test_native_public_category_and_product_extensions(self):
        ...
        self.assertNotIn("dtf_public_published", product._fields)
    ...
    def test_native_variants_and_stock_are_used(self):
        ...
        product = self.env["product.template"].create({
            "name": "Variant Shirt",
            "attribute_line_ids": [(0, 0, {"attribute_id": attribute.id, "value_ids": [(6, 0, [value.id])]})],
        })
    ...
    def test_printify_mapping_is_unique_and_unpublished_by_native_field(self):
