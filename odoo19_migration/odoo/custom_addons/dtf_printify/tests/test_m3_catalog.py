...
class TestDTFM3Catalog(TransactionCase):
    ...
    def test_bilingual_site_category_and_native_product(self):
        category = self.env["dtf.site.category"].create({"name_en": "T-Shirts", "name_ar": "قمصان", "description_en": "Shirts", "description_ar": "قمصان", "slug": "t-shirts"})
        product = self.env["product.template"].create({"name": "DTF Shirt", "dtf_name_en": "DTF Shirt", "dtf_name_ar": "قميص DTF", "dtf_site_category_id": category.id, "dtf_product_type": "tshirt", "dtf_catalog_type": "customizable", "dtf_print_your_dream_eligible": True, "dtf_designer_design_compatible": True, "dtf_public_published": True, "list_price": 12.5})
        self.assertEqual(product.dtf_site_category_id, category)
    ...
    def test_snapshot_import_is_idempotent_and_does_not_publish(self):
        ...
        self.assertFalse(self.env["product.template"].search([("dtf_printify_source_id", "=", "bp-42")], limit=1).dtf_public_published)
