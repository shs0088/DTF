...
| M5 cart/API | Manual sale order/line creation and lst_price assignment | website_sale sale_get_order, _cart_add, _cart_update_line_quantity, computed totals | DUPLICATE CUSTOM LOGIC | Replaced business logic with thin native website-sale adapter | None | dtf_api/controllers/api.py | Frontend JSON envelope only | Source inspection plus workflow |
...
- Full runtime proof for every headless website-sale/payment/delivery route
- Whether dtf.site.category should later migrate fully to product.public.category
